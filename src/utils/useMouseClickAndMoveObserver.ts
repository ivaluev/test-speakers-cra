...
export default function useMouseClickAndMoveObserver(targetRef: React.RefObject<HTMLElement>) {
  const [mouseState, setMouseState] = useState<MouseEventType>(MouseEventType.NONE);
  ...
  }, [mouseMovingRect]); // eslint-disable-line
  ...
  const onMouseMove = useCallback((e: MouseEvent): any => {
    ...
    const currentRectInfo = new RectInfo(mouseMovingRect.x1, mouseMovingRect.y1, e.offsetX, e.offsetY);
